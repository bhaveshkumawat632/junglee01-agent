...
@app.post("/api/autogen/conversation")
async def autogen_conversation(request: Request):
    body = await request.json()
    task_prompt = body.get("task_prompt", "Collaborative task")
    agents = body.get("agents", ["Assistant", "User"])
    task_id = f"autogen_{int(time.time())}_{uuid.uuid4().hex[:8]}"
    now = datetime.now(timezone.utc).isoformat()
    conn = sqlite3.connect(DB_PATH)
    conn.execute(
        "INSERT INTO tasks (task_id, task_text, status, model, created_at, updated_at) VALUES (?, ?, 'pending', ?, ?, ?)",
        (task_id, f"autogen:{task_prompt[:40]}", "autogen", now, now)
    )
    conn.commit()
    conn.close()
    asyncio.create_task(run_autogen_conversation(task_id, task_prompt, agents))
    return JSONResponse({"task_id": task_id, "task_prompt": task_prompt, "agents": agents})

async def run_autogen_conversation(task_id: str, task_prompt: str, agents: list):
    lines = [f"🤖 AutoGen conversation: {len(agents)} agents", f"📝 Task: {task_prompt[:50]}"]
    for i, line in enumerate(lines, 1):
